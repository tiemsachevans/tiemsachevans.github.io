      ...
      if (puzzleId === "huy") {
        ...
        if (hintBox) {
          hintBox.innerHTML = `
            ...
            <hr style="border: 0; border-top: 1px dashed #d4af37; margin: 10px 0;">
            <span style="font-style: italic; color: #8b3a3a; font-size: 0.75rem; line-height: 1.4;">Giải pass thử sức hoặc đọc phần "Tâm Thư Chủ Tiệm" ở Trang chủ để nhận link nhé! ദ്ദി≽^-⩊•^≼ ✧</span>
          `;
        }
  ...
  if (validAnswers.includes(code)) {
    ...
    if (puzzleId === "huy") {
      ...
      if (huyWrongAttempts === 1) {
        ...
        errorMsg.innerHTML = `<span style="font-family: 'Merriweather', serif; color: #8b3a3a; font-weight: 600; font-size: 0.88rem; display: block; line-height: 1.4;">ฅ₍^>⩊<^₎ฅ⟆ Gợi ý từ Mimi:<br>Lữ khách đã kiểm tra tài khoản Instagram của Dương Khắc Huy chưa~?</span>`;
      }
    } else {
...
async function syncAccountPuzzles(user) {
  ...
  try {
    ...
    let localMap = getLocalUnlockedPuzzlesMap();

    // Hợp nhất dữ liệu giữa Cloud (Supabase) và LocalStorage (ưu tiên kết hợp cả 2 chiều tránh mất mát)
    const mergedMap = { ...cloudPuzzles, ...localMap };

    // Cập nhật ngược lại vào LocalStorage
    Object.keys(mergedMap).forEach(puzzleId => {
      if (mergedMap[puzzleId]) {
        localStorage.setItem(`unlocked_${puzzleId}`, "true");
      }
    });

    // Đẩy dữ liệu đồng nhất lên lại Supabase
    await supabase
      .from("profiles")
      .upsert({
        user_id: user.id,
